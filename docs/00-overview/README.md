...
1. `src/Incursa.OpenAI.ChatKit/ChatKitServer.cs`
   - central request dispatcher and streaming/non-streaming split
2. `src/Incursa.OpenAI.ChatKit/Store.cs`
   - persistence contract for threads, items, and attachments
3. `src/Incursa.OpenAI.ChatKit/ChatKitRequests.cs`
   - supported request types and their parameter envelopes
4. `src/Incursa.OpenAI.ChatKit/ChatKitEvents.cs`
   - stream event types produced during assistant responses
5. `src/Incursa.OpenAI.ChatKit.AspNetCore/ChatKitEndpointRouteBuilderExtensions.cs`
   - HTTP adapter that maps raw JSON to the core runtime
6. `src/Incursa.OpenAI.ChatKit.AspNetCore/TagHelpers`
...
- request type supported by `ChatKitRequest`, but not yet exercised end to end
