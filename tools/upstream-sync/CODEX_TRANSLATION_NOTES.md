...
- This automation is translating only the behavior introduced in an upstream diff from `chatkit-js`.
...
- Prefer tests that mirror the upstream JS change and keep the quickstart/docs aligned when public behavior changes.
- Keep modifications minimal and frame them so reviewers can trace them directly to the upstream JS changes.
